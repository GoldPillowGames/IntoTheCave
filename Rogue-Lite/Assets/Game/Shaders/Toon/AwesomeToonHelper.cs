...
namespace AwesomeToon {
    ...
    public class AwesomeToonHelper : MonoBehaviour {
        ...
        void Init() {
            ...
            if (instanceMaterial) {
                materialInstance = new Material[material.Length];
                for(int i = 0; i < material.Length; i++)
                {
                    materialInstance[i] = new Material(material[i]);
                    materialInstance[i].name = "Instance of " + material[i].name;
                }
            ...
            if (skinRenderer) skinRenderer.sharedMaterials = materialInstance;
            // if (meshRenderer) meshRenderer.sharedMaterial = materialInstance;
            if (meshRenderer) meshRenderer.sharedMaterials = materialInstance;
        }
        ...
        void UpdateMaterial() {
            ...
            foreach (LightSet lightSet in sortedLights) {
                ...
                {
                    materialInstance[j].SetVector($"_L{i}_dir", lightSet.dir.normalized);
                }
                // materialInstance.SetColor($"_L{i}_color", color);
                for (int j = 0; j < material.Length; j++)
                {
                    materialInstance[j].SetColor($"_L{i}_color", color);
                }
                i++;
            }
            ...
            while (i <= maxLights) {
                ...
                for (int j = 0; j < material.Length; j++)
                {
                    materialInstance[j].SetVector($"_L{i}_dir", Vector3.up);
                }
                // materialInstance.SetColor($"_L{i}_color", Color.black);
                for (int j = 0; j < material.Length; j++)
                {
                    materialInstance[j].SetColor($"_L{i}_color", Color.black);
                }
                i++;
            }
